...
以下 `curl` 命令可以用来测试本示例中运行的 CORS 服务器。请将 `http://your-ip:8080` 替换为你的实际地址。
...
```bash
# 模拟来自一个允许的源的请求
curl -X GET http://your-ip:8080/api/cors/simple \
...
curl -X OPTIONS http://your-ip:8080/api/cors/preflight \
  -H "Origin: http://any-origin.com" \
...
curl -X POST http://your-ip:8080/api/cors/preflight \
  -H "Origin: http://any-origin.com" \
